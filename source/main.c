...
/* ... */
	...
	{ .pt_enable = phi_physmem_enable, .pt_disable = phi_physmem_disable, .t_status = 0, .pt_module_string = "physmem" },
	{ .pt_enable = phi_paging_enable, .pt_disable = phi_paging_disable, .t_status = 0, .pt_module_string = "paging" },
...
/* ... */
uint32_t g_module_buffer_size = sizeof(g_module_buffer) / sizeof(g_module_buffer[0]);
...
{
/* ... */
	for(uint32_t l_i = 0; l_i < g_module_buffer_size; l_i++) g_module_buffer[l_i].t_status = g_module_buffer[l_i].pt_enable();
...
/* ... */
	for(uint32_t l_i = 0; l_i < g_module_buffer_size; l_i++) if(g_module_buffer[l_i].t_status == 0) g_module_buffer[l_i].t_status = g_module_buffer[l_i].pt_disable();
